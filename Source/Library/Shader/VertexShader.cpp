...
namespace library
{
    ...
    HRESULT VertexShader::Initialize(_In_ ID3D11Device* pDevice)
    {
        ...
        hr = pDevice->CreateVertexShader(pVSBlob->GetBufferPointer(),
            pVSBlob->GetBufferSize(), nullptr, m_vertexShader.GetAddressOf());
        if (FAILED(hr))
        {
            return hr;
        }
        ...
        UINT uNumElements = ARRAYSIZE(aLayout);
        
        // Create the input layout
        hr = pDevice->CreateInputLayout(aLayout, uNumElements, pVSBlob->GetBufferPointer(),
